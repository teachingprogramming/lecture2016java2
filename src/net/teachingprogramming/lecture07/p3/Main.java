package net.teachingprogramming.lecture07.p3;
...
public class Main {
    public static void main(String[] args) {
        ...
        // 回答ここから
        HashSet<String> wordSet = new HashSet<String>(); // 3文字以下の単語を格納するセット
        for (String word : wordList) {
            if (word.length() <= 3) {
                wordSet.add(word);
            }
        }
        ArrayList<String> sortedList = new ArrayList<String>(); // ソートされたリスト（まだここではソートされていない）
        sortedList.addAll(wordSet); // セットの中身をsortedListに加える
        Collections.sort(sortedList); // sortedListをソート
